...
CREATE TABLE `permissions` ( 
  ...
  `id_group` VARCHAR(20) NOT NULL,
   PRIMARY KEY (`id`),
  CONSTRAINT `UQ_permissions_endpoint` UNIQUE (`endpoint`)
)
ENGINE = InnoDB;
...
ENGINE = InnoDB;
CREATE INDEX `IX_changes_row_id` 
ON `changes` (
  `row_id` ASC
);
...
CREATE INDEX `IX_tables_columns_length` 
ON `tables_columns` (
  `length` ASC
);
...
ALTER TABLE `changes` ADD CONSTRAINT `FK_changes_id_table` FOREIGN KEY (`id_table`) REFERENCES `tables` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;
...
ALTER TABLE `views_sort` ADD CONSTRAINT `FK_views_sort_id_view` FOREIGN KEY (`id_view`) REFERENCES `views` (`identifier`) ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE `views_sort` ADD CONSTRAINT `FK_views_sort_id_column` FOREIGN KEY (`id_column`) REFERENCES `tables_columns` (`identifier`) ON DELETE CASCADE ON UPDATE CASCADE;
